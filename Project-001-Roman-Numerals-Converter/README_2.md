...
aws ec2 authorize-security-group-ingress \
    --group-name roman_numbers-temelsec \
    --protocol tcp \
    --port 22 \
    --cidr 0.0.0.0/0
...
aws ec2 authorize-security-group-ingress --group-name  sg-903004f8 --protocol tcp --port 3389 --cidr x.x.x.x/x
...
pip3 install flask
cd /home/ec2-user
FOLDER="https://raw.githubusercontent.com/mustafa69-code/projelerim-aws/main/Project-001-Roman-Numerals-Converter/"
wget ${FOLDER}/app.py
...
```
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name first-key-pair --security-group-ids  sg-0ef5ca7fe6bedfb91 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
aws ec2 run-instances \
    ...
    --security-group-ids sg-0ef5ca7fe6bedfb91 \
